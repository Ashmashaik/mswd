...
function removeun() {
    axios.get('http://localhost:8082/removeun', {params:{
    }}).then((response)=>{
    console.log(response.data)
    setResult(response.data)
    })
...
function sortas() {
    axios.get('http://localhost:8082/sortas', {params:{
    }}).then((response)=>{
    console.log(response.data)
    setResult(response.data)
    })
...
if(result == null) {
    return(
        <div>
            ...
            <br/>
            <button onClick={()=>showOne()}>Display First Student in 
            Database</button>
            <br/>
            <button onClick={()=>showAll()}>Display All Students</button>
            <br/>
            <button onClick={()=>showAllGt1()}>Display Student Value id 
            </button>
            <br/>
            <button onClick={()=>removeun()}>Delete all students data with 
            invalid branch</button>
            <br/>
            <button onClick={()=>sortas()}>Sort the students data in 
